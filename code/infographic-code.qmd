...
The box and scatter plots were visualizations we've become familiar with in class, but the icons were inspired by an affective visualization I saw in our last section on Friday. A classmate had used birds to represent raw data points, and the buildings on which the birds sat also represented data points. I wanted to try something similar, at least in terms of adding icons to my figures. For both figures, I cleaned the data and used geom_image to place imported icons onto the plots. The box plot used geom_boxplot.

I troubleshooted errors in my code through forums on StackOverflow (I was relieved to see that others had run into the same issues I did and sought help from experts), watched tutorials on YouTube, and read articles (mostly for showtext and the pie donut) in many attempts to make the code work.